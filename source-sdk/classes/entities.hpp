...
public:
	...
	vec3_t get_hitbox( int hitbox ) {
		...
		if ( this->setup_bones( matrix, 128, 256, interfaces::globals->cur_time ) ) {
			...
			if ( box ) {
				...
				math_temp->vec3_transform( box->mins, matrix[ box->bone ], min );
				math_temp->vec3_transform( box->maxs, matrix[ box->bone ], max );
	...
	matrix_t &coord_frame( ) {
		const static auto m_CollisionGroup = netvar_manager::get( ).get_offset( fnv_hash( "DT_BaseEntity->m_CollisionGroup") );
	...
	netvar_fn( bool, client_side_anim, "DT_BaseAnimating->m_bClientSideAnimation" );
	...
	netvar_fn( vec3_t, velocity, "DT_CSPlayer->m_vecVelocity" );
};
...
public:
	...
	bool is_c4( ) {
		return clientclass( )->class_id == cc4;
	}
	std::string name( ) {
		auto ret = [ ] ( const char *in ) -> const char* {
			if ( in[ 0 ] == 'C' )
				in++;

			auto start = strstr( in, "weapon_" );
			if ( start != nullptr )
				in = start + 7;
		...
		return ret( this->get_weapon_data( )->weapon_name );
	}
};
...
public:
	...
	netvar_fn( int, money, "DT_CSPlayer->m_iAccount" );

	int &get_take_damage( ) {
		return *reinterpret_cast< int * >( uintptr_t( this ) + 0x27C );
	}
	...
	move_type movetype( ) {
		return *(move_type*)((DWORD)this + 0x25C);
	}
};